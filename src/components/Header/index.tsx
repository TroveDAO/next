...
import { getNonce, login } from '../../lib/login'
...
export const Header = () => {
  ...
  const [{ data: signature, error, loading }, signMessage] = useSignMessage()
  const [clicked, setClicked] = useState(false);
  const loginBro = async () => {
    setClicked(true)
    const nonce = await getNonce(accountData.address);
    const message = `Ask and you shall receive - ${nonce.nonce}`
    await signMessage({ message })
  }

  useEffect(() => {
    const loginFinally = async () => {
      const user = await login(accountData.address, signature);
      console.log(user)
      const is = await isLoggedIn();
      setIsLogged(is)
    }
    if(!loading && clicked) {
      loginFinally();
            ...
            <NavButton ml="30px" onClick={() => connect(connector)}>
              {accountData && accountData.address ? (
                ...
                {isLogged ? 
                  ...
                  <NavButton ml="30px" onClick={async () => {
